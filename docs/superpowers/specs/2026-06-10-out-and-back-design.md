...
One pure transform in the GPX library, applied in the page *before* all existing derivations. Everything downstream (`computeSummary`, lap distance, `repeat`, download) consumes the transformed GPX and needs no awareness of the feature.
...
- Point `trackSummary`, `lapDistanceMeters`, `summaryView`, and `handleDownload`'s `repeat()` call at `effectiveGpx`. The DropZone's `loadedSummary` keeps showing the *original* file's one-way stats (the callout communicates the conversion; the Review section shows the result).
